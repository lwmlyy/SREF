...
if __name__ == '__main__':
    ...
    parser.add_argument('-sup_sv_path', help='Path to supervised sense vectors', required=False, default='data/vectors/semcor.txt')
    parser.add_argument('-ext_mode', default='lexname', help='Max abstraction level', required=False,
                        choices=['synset', 'hypernym', 'lexname'])
    parser.add_argument('-out_path', help='Path to resulting extended vector set', required=False, default='data/vectors/lmms.txt')
    args = parser.parse_args()

    logging.info('Loading SensesVSM ...')
    import pickle
    sense_dict = pickle.load(open(args.sup_sv_path, 'rb'))
    senses_vsm = SensesVSM(sense_dict, normalize=False)
    ...
    sense_dict.update(additional_vecs)
